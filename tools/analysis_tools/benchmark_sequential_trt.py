...
def main():
    ...
    for i, data in enumerate(data_loader):
        if init_:
            ...
            metas_, sensor2keyegos, bda = model.get_bev_pool_input(inputs)
            metas = dict(
            ...
            init_ = False
        # prepare features from previous frame
        img_prev = data['img_inputs'][0][1].cuda().squeeze(0).contiguous()
        feat_prev = trt_model.prepare_prev_bev_feat(dict(img=img_prev, **metas))
        
        # alight feature from previous frame 
        feat_prev = model.shift_feature(feat_prev,
        ...
        trt_output = trt_model.forward(dict(img=img,feat_prev=feat_prev, **metas))
